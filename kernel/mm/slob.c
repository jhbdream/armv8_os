...
/* --------------------------------------------------------------------------*/
/**
 * @brief  在空闲slab中存放一些标记,让空间单元链接成一个单链表结构
 *
 *
                             ...
                             int align_offset, bool *page_remove_from_list)
{
    ...
    *page_remove_from_list = false;
    ...
    for (prev = NULL, cur = slob_page->freelist;; prev = cur, cur = slob_next(cur)) {
        ...
        if (align) {
            aligned =
            (slob_t *)((ALIGN((unsigned long)cur + align_offset, align)) - align_offset);
...
void __kfree(const void *block)
{
    ...
    if (PageSlab(page)) {
        m = (unsigned long *)(block - SLOB_ALIGN_OFFSET);
...
#define SLOB_TEST_SIZE  61
    ...
    for (i = 0; i < SLOB_TEST_CYCLE; i++) {
        addr[i] = kmalloc(i);
        printk("[%04d] kmalloc addr: [0x%016lx]\n", i, addr[i]);
    }

    for (i = 0; i < SLOB_TEST_CYCLE; i++) {
        kfree(addr[i]);
        printk("[%04d]   kfree addr: [0x%016lx]\n", i, addr[i]);
    }
}
