			   ...
			   phys_addr_t (*pgtable_alloc)(int), int flags)
{
	...
	do {
		...
		if (use_1G_block(addr, next, phys)) {
			...
			alloc_init_pmd(pudp, addr, end, phys, prot,
				       pgtable_alloc, flags);
		}
...
static void map_kernel(pgd_t *pgdp)
{
	...
	create_pgd_mapping(pgdp, pa, va, size, PAGE_KERNEL_EXEC,
...
void paging_init(void)
{
	...
	create_pgd_mapping(swapper_pg_dir, 0x09000000, 0xFFFF100000000000,
			   0x1000, PAGE_DEVICE, early_pgtable_alloc, 0);
