...
static inline void *gic_data_rdist_rd_base(void)
{
     ...
     */
    aff = (MPIDR_AFFINITY_LEVEL(mpidr, 3) << 24 | MPIDR_AFFINITY_LEVEL(mpidr, 2) << 16 |
...
int gic_irq_get_irqchip_state(u32 hwirq, enum irqchip_irq_state which, bool *val)
{
    switch (which) {
    case IRQCHIP_STATE_PENDING:
...
static u64 gic_mpidr_to_affinity(unsigned long mpidr)
{
    ...
    aff =
    ((u64)MPIDR_AFFINITY_LEVEL(mpidr, 3) << 32 | MPIDR_AFFINITY_LEVEL(mpidr, 2) << 16 |
     MPIDR_AFFINITY_LEVEL(mpidr, 1) << 8 | MPIDR_AFFINITY_LEVEL(mpidr, 0));
...
int gic_init_bases(void *dist_base, void *rdist_base, u32 nr_redist_regions)
{
    u32 typer;
    int gic_irqs;
    int i, err;
