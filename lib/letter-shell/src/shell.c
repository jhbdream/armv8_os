...
void shellInit(Shell *shell, char *buffer, unsigned short size)
{
	...
	for (short i = 0; i < SHELL_HISTORY_MAX_NUMBER; i++) {
		shell->history.item[i] =
			buffer + shell->parser.bufferSize * (i + 1);
	}
...
static void shellWritePrompt(Shell *shell, unsigned char newline)
{
	if (shell->status.isChecked) {
		...
		shellWriteString(shell, ":");
		shellWriteString(shell,
				 shell->info.path ? shell->info.path : "/");
		shellWriteString(shell, "$ ");
	} else {
...
void shellListItem(Shell *shell, ShellCommand *item)
{
	...
	for (signed char i = 7; i >= 0; i--) {
		shellWriteByte(shell, item->attr.attrs.permission & (1 << i) ?
					      'x' :
					      '-');
	}
#endif
...
void shellInsertByte(Shell *shell, char data)
{
	...
	if (shell->parser.cursor == shell->parser.length) {
		...
		for (short i = shell->parser.length - shell->parser.cursor;
		     i > 0; i--) {
			shell->parser.buffer[shell->parser.cursor + i] =
				shell->parser
					.buffer[shell->parser.cursor + i - 1];
		}
		shell->parser.buffer[shell->parser.cursor++] = data;
		shell->parser.buffer[++shell->parser.length] = 0;
		for (short i = shell->parser.cursor - 1;
		     i < shell->parser.length; i++) {
...
void shellDeleteByte(Shell *shell, signed char direction)
{
	...
	if (shell->parser.cursor == shell->parser.length && direction == 1) {
		...
		for (short i = offset;
		     i < shell->parser.length - shell->parser.cursor; i++) {
		...
		shell->parser.buffer[shell->parser.length] = 0;
		for (short i = shell->parser.cursor; i < shell->parser.length;
		     i++) {
			shellWriteByte(shell, shell->parser.buffer[i]);
		}
		shellWriteByte(shell, ' ');
...
static void shellParserParam(Shell *shell)
{
	...
	for (unsigned short i = 0; i < shell->parser.length; i++) {
		if (quotes != 0 || (shell->parser.buffer[i] != ' ' &&
				    shell->parser.buffer[i] != 0)) {
			if (shell->parser.buffer[i] == '\"') {
				quotes = quotes ? 0 : 1;
			}
			if (record == 1) {
				if (shell->parser.paramCount <
			       ...
			       ShellCommand *base, unsigned short compareLength)
{
	...
	unsigned short count =
		shell->commandList.count -
		((unsigned long)base - (unsigned long)shell->commandList.base) /
			sizeof(ShellCommand);
	...
	case SHELL_TYPE_VAR_NODE:
		value = ((ShellNodeVarAttr *)command->data.var.value)->get ?
				((ShellNodeVarAttr *)command->data.var.value)
					->get(((ShellNodeVarAttr *)
						       command->data.var.value)
						      ->var) :
				0;
		break;
	default:
	...
	if (command->attr.attrs.readOnly) {
		...
		case SHELL_TYPE_VAR_NODE:
			if (((ShellNodeVarAttr *)command->data.var.value)->set) {
				if (((ShellNodeVarAttr *)command->data.var.value)
					    ->var) {
					((ShellNodeVarAttr *)
						 command->data.var.value)
						->set(((ShellNodeVarAttr *)
							       command->data.var
								       .value)
							      ->var,
						      value);
...
static int shellShowVar(Shell *shell, ShellCommand *command)
{
	...
	default:
		shellWriteString(shell,
				 &buffer[11 - shellToDec(value, buffer)]);
...
unsigned int shellRunCommand(Shell *shell, ShellCommand *command)
{
	...
	if (command->attr.attrs.type == SHELL_TYPE_CMD_MAIN) {
		...
		returnValue = command->data.cmd.function(
			shell->parser.paramCount, shell->parser.param);
		...
		returnValue =
			shellExtRun(shell, command, shell->parser.paramCount,
				    shell->parser.param);
		if (!command->attr.attrs.disableReturn) {
			shellWriteReturnValue(shell, returnValue);
		}
...
{
	if (strcmp(shell->parser.buffer,
		   shell->info.user->data.user.password) == 0) {
		shell->status.isChecked = 1;
#if SHELL_SHOW_INFO == 1
...
static void shellSetUser(Shell *shell, const ShellCommand *user)
{
	...
	shell->status.isChecked = ((user->data.user.password &&
				    strlen(user->data.user.password) != 0) &&
				   (shell->parser.paramCount < 2 ||
				    strcmp(user->data.user.password,
					   shell->parser.param[1]) != 0)) ?
					  0 :
					  1;
...
static void shellHistory(Shell *shell, signed char dir)
{
	...
	if (shell->history.offset == 0) {
		...
		if ((shell->parser.length = shellStringCopy(
			     shell->parser.buffer,
			     shell->history.item[(shell->history.record +
						  SHELL_HISTORY_MAX_NUMBER +
						  shell->history.offset) %
						 SHELL_HISTORY_MAX_NUMBER])) ==
		    0) {
...
void shellExec(Shell *shell)
{
	...
	if (shell->status.isChecked) {
		...
		ShellCommand *command =
			shellSeekCommand(shell, shell->parser.param[0],
					 shell->commandList.base, 0);
		if (command != NULL) {
			shellRunCommand(shell, command);
		} else {
			shellWriteString(shell,
					 shellText[SHELL_TEXT_CMD_NOT_FOUND]);
		}
	} else {
...
void shellRight(Shell *shell)
{
	if (shell->parser.cursor < shell->parser.length) {
		shellWriteByte(shell,
			       shell->parser.buffer[shell->parser.cursor++]);
	}
}
			...
			if (shellCheckPermission(shell, &base[i]) == 0 &&
				...
				if (matchNum != 0) {
					...
					length = shellStringCompare(
						(char *)shellGetCommandName(
							&base[lastMatchIndex]),
						(char *)shellGetCommandName(
							&base[i]));
					maxMatch = (maxMatch > length) ?
							   length :
							   maxMatch;
				}
				lastMatchIndex = i;
...
void shellHandler(Shell *shell, char data)
{
	...
	if (shell->info.user->data.user.password &&
	    ...
	    SHELL_GET_TICK()) {
		if (SHELL_GET_TICK() - shell->info.activeTime >
		    SHELL_LOCK_TIMEOUT) {
