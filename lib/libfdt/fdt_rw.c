                                ...
                                struct fdt_property **prop)
{
    ...
    *prop = fdt_get_property_w(fdt, nodeoffset, name, &oldlen);
    ...
    if ((err =
         fdt_splice_struct_(fdt, (*prop)->data, FDT_TAGALIGN(oldlen), FDT_TAGALIGN(len))))
        return err;
...
int fdt_set_name(void *fdt, int nodeoffset, const char *name)
{
    ...
    err =
    fdt_splice_struct_(fdt, namep, FDT_TAGALIGN(oldlen + 1), FDT_TAGALIGN(newlen + 1));
    if (err)
        return err;
...
int fdt_appendprop(void *fdt, int nodeoffset, const char *name, const void *val, int len)
{
    ...
    if (prop) {
        newlen = len + oldlen;
        err =
        fdt_splice_struct_(fdt, prop->data, FDT_TAGALIGN(oldlen), FDT_TAGALIGN(newlen));
        if (err)
            return err;
