                                                ...
                                                uint32_t delta)
{
    ...
    fdt_for_each_property_offset(fixup_prop, fdto, fixup_node)
    {
        ...
        for (i = 0; i < (fixup_len / sizeof(uint32_t)); i++) {
            ...
            ret = fdt_setprop_inplace_namelen_partial(fdto, tree_node, name, strlen(name),
                                                      poffset, &adj_val, sizeof(adj_val));
...
static int overlay_fixup_one_phandle(void *fdt, void *fdto, int symbols_off,
                                     ...
                                     const char *name, uint32_t name_len, int poffset,
                                     const char *label)
{
...
static int overlay_fixup_phandle(void *fdt, void *fdto, int symbols_off, int property)
{
    ...
    do {
        ...
        ret = overlay_fixup_one_phandle(fdt, fdto, symbols_off, path, path_len, name,
                                        name_len, poffset, label);
        if (ret)
            return ret;
    } while (len > 0);
